...
void OpticFlow::timerCamInit([[maybe_unused]] const ros::TimerEvent& event) {
  ...
  if (!got_image_) {
    ...
    // ROS_INFO_THROTTLE(1.0, "[OpticFlow]: (timerCamInit) waiting for camera");
  ...
  if (!got_camera_info_ && ((ros::Time::now() - camera_info_timeout_).toSec() < 15.0)) {
    ...
    // ROS_INFO_THROTTLE(1.0, "[OpticFlow]: (timerCamInit) waiting for camera info");
    return;
  }
  ...
  try {
    {
      ...
      ROS_INFO("HERE AM I1 %s %s", &_uav_frame_[0], &_camera_frame_[0]);
    ...
    ROS_INFO("HERE AM I too 1");
    ROS_ERROR("[OpticFlow]: TF: %s", ex.what());
    ROS_INFO("HERE AM I too 2");
...
void OpticFlow::callbackOdometry(const nav_msgs::OdometryConstPtr& msg) {
  ...
  // ROS_INFO_THROTTLE(1.0, "[OpticFlow]: (callbackOdometry) here am I");
  if (!is_initialized_) {
    // ROS_INFO_THROTTLE(1.0, "[OpticFlow]: (callbackOdometry) you're gonna lose this girl");
    return;
  }
  // ROS_INFO_THROTTLE(1.0, "[OpticFlow]: (callbackOdometry) gonna set everything OK");
...
void OpticFlow::callbackImage(const sensor_msgs::ImageConstPtr& msg) {
  ...
  if ((!got_c2b_) || (!got_b2c_)) {
    if (!got_c2b_) {
      ROS_INFO_THROTTLE(1.0, "[OpticFlow]: waiting for transform Base - Camera (c2b)");
    } else if (!got_b2c_) {
      ROS_INFO_THROTTLE(1.0, "[OpticFlow]: waiting for transform Base - Camera (b2c)");
    } else {
      ROS_INFO_THROTTLE(1.0, "[OpticFlow]: waiting for transform Base - Camera (wtf)");
    } 
    return;
  }
...
void OpticFlow::processImage(const cv_bridge::CvImagePtr image) {
  ...
  /* ROS_INFO("[OpticFlow]: got all we need"); */
  ...
  // scale the image
  //ROS_INFO("[OpticFlow]: scale the image (scale factor is %i)", _scale_factor_);
  if (fabs(_scale_factor_ - 1.0) > 0.01) {
  ...
  // cropping
  //ROS_INFO("[OpticFlow]: crop the image");
  ...
  //ROS_INFO("[OpticFlow]: im cntr x %i y %i", image_center_x, image_center_y);
  //ROS_INFO("[OpticFlow]: im cntr x %i y %i", (image_scaled_.size().width / 2), (image_scaled_.size().height / 2));

  // rectification
  //ROS_INFO("[OpticFlow]: rectificate the image");
  ...
  //ROS_INFO("[OpticFlow]: convert the image to grayscale");
  /* if (first_image_) */
  try {
    //ROS_INFO("[OpticFlow]: 1");
    //ROS_INFO("[OpticFlow]: %i %i %s",
    //  image_scaled_.size().width,
    //  image_scaled_.size().height,
    //  typeid(image_scaled_).name());
    //ROS_INFO("[OpticFlow]: %i %i %s",
    //  cropping_rectangle.size().width,
    //  cropping_rectangle.size().height,
    //  typeid(cropping_rectangle).name());
    //image_scaled_(cropping_rectangle);
    //ROS_INFO("[OpticFlow]: 2");
    //ROS_INFO("[OpticFlow]: %i %i", image_scaled_(cropping_rectangle).size().width, image_scaled_.size().height);
    //ROS_INFO("[OpticFlow]: 3");
    ...
    //  ROS_INFO("[OpticFlow]: made the lol window");
    //}

    cv::cvtColor(image_scaled_(cropping_rectangle), imCurr_, CV_RGB2GRAY);

    if (_gui_) {
    	cv::imshow("lol", image_scaled_(cropping_rectangle));
    	cv::waitKey(1);
    }

  } catch(const std::exception& ex) {
    //ROS_INFO("[OpticFlow]: exception: %s", ex.what());
    return;
    throw;
  }
  ...
  //ROS_INFO("[OpticFlow]: estimate scale and rotation of the image (if enabled)");
  ...
  // process image
  /* ROS_INFO("[OpticFlow]: process the image"); */
